...
echo -e -n '#!/bin/bash\nwp.phar "$@"' > bin/wp
chmod +x bin/wp
echo -e -n 'php %~dp0wp.phar %*' > bin/wp.bat
...
echo -e -n 'php %~dp0composer.phar %*' > bin/composer.bat
...
chmod +x bin/phpunit
echo -e -n 'php %~dp0phpunit.phar %*' > bin/phpunit.bat
...
echo -e -n 'php --define phar.readonly=0 %~dp0phar-composer.phar %*' > bin/phar-composer.bat
