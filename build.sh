...
echo -e -n '#!/bin/bash\nwp.phar "$@"' > bin/wp
chmod +x bin/wp
echo -e -n 'php -f wp.phar %*' > bin/wp.bat
...
echo -e -n 'php -f composer.phar %*' > bin/composer.bat
...
chmod +x bin/phpunit
echo -e -n 'php -f phpunit.phar %*' > bin/phpunit.bat
...
echo -e -n 'php --define phar.readonly=0 -f phar-composer.phar %*' > bin/phar-composer.bat
